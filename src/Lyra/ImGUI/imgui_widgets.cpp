...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col, ImVec2 pos, float size, float spacing, bool sv, bool hv)
{
    ImGuiContext& g = *GImGui;
    //ImGuiWindow* window = GetCurrentWindow();
    //if (window->SkipItems)
    ...
    ImVec2 picker_pos = pos;
    float square_sz = size;
    float bars_width = square_sz/6; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = size;//ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    float bar0_pos_x = picker_pos.x + sv_picker_size + spacing;
