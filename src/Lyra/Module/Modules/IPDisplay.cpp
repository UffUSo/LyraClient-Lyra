...
IPDisplay::IPDisplay() : ModuleCtor(0, "IP Display", "IPDisplay") {
	this->InitModuleSettings();
}

std::string IPDisplay::getModuleName() {
	return "Server IP";
}

void IPDisplay::InitModuleSettings() {
...
void IPDisplay::onRender(const RenderEvent& event) {
    if(!SDK::clientInstance || !SDK::clientInstance->getLocalPlayer()) return;
    if (!(SDK::TopScreen.rfind("hud_screen") != std::string::npos)) return;
    auto ip = SDK::clientInstance->getRakNetConnector()->JoinedIp;
    if(ip.empty()) ip = "localhost";
	this->RenderHUD(ip);
}
void IPDisplay::RenderHUD(std::string text) {
